...
# Mapping of stage codes to full names
stage_full_names = {
...
# Show info side-by-side
info_col1, info_col2 = st.columns(2)

def show_club_info_in_col(club, container):
    with container:
        img_col1, img_col2, txt_col = st.columns([1, 0.75, 3])
        with img_col1:
            if club.get("crest_url"):
                st.image(club["crest_url"], width=120)
        with img_col2:
            comp_logo = competition_logos.get(club.get("competition"), None)
            if comp_logo:
                st.image(comp_logo, width=90)  # 75% of crest width
        with txt_col:
            entry_stage = club.get("entry_stage", "Unknown")
            full_stage = stage_full_names.get(entry_stage, entry_stage)
            st.markdown(f"**Entry Stage:** {full_stage}")
            st.markdown(f"**Competition:** {club.get('competition', 'Unknown')}")

show_club_info_in_col(club1, info_col1)
show_club_info_in_col(club2, info_col2)

# Simple meeting logic (expand as needed)
